...
/**
 * Sample the ring palette at t in [0,1]. Upstream mixes exactly two colours;
 * this walks a list of up to eight so the palette is admin-editable.
 *
 * The pair is picked by a constant-bounded loop rather than uColors[idx]:
 * GLSL ES 1.00 forbids indexing a uniform array with a non-constant expression
 * in a fragment shader, and ShaderMaterial compiles as ES 1.00 by default.
 */
...
/** Must match the `uColors[8]` declaration in the fragment shader. */
const MAX_COLORS = 8;
...
export default function MagicRings({
  ...
  colorTwo = '#42fcff',
  /**
   * Ordered palette the rings interpolate across, innermost to outermost.
   * Overrides `color`/`colorTwo` when given; anything past the eighth entry is
   * ignored because the shader's uniform array is fixed-size.
   */
  ...
  // The render loop is built once and reads the current props from this ref,
  // which is refreshed after every render. Upstream assigns it during render;
  // an effect does the same job without touching a ref mid-render.
  const propsRef = useRef({
    color, colorTwo, colors, speed, ringCount, attenuation, lineThickness,
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uColors: {
        value: Array.from({ length: MAX_COLORS }, () => new THREE.Color()),
      },
      uColorCount: { value: 2 },
    ...
    const animate = (t) => {
      ...
      // `.set()` in place — allocating Colors every frame would churn the heap
      // for a value that changes only when an admin picks a new palette.
      const palette =
        p.colors && p.colors.length > 0 ? p.colors : [p.color, p.colorTwo];
      const count = Math.min(palette.length, MAX_COLORS);
      for (let i = 0; i < count; i++) uniforms.uColors.value[i].set(palette[i]);
