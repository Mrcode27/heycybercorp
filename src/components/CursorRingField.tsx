 ...
 *
 * WHY GPGPU AND NOT A PURE VERTEX-SHADER SCENE. Most scenes compute a point's
 * position from time and a seed alone, so nothing has to be remembered. This
 * one leaks: each frame a point keeps 80% of last frame's displacement, its
 * scale eases toward the ring band, and its "energy" is a decaying accumulator
 * of that scale. That history lives in a floating-point texture — one texel per
 * point, `xy` position, `z` scale, `w` energy — integrated by a fragment shader
 * into a second texture which is then swapped in. The CPU never touches a
 * particle.
 *
 ...
 *
 * THE INTERACTION IS THE SOURCE'S. The camera is fixed. The pointer moves the
 * RING, never the camera — there is no orbit, no drag, no gesture state. The
 * pointer is ray-cast onto the field plane and the ring eases toward that point
 * on a 130ms time constant, with a small noise jitter riding on top. With no
 * pointer the ring wanders on two slow 1D value-noise walks instead, so the
 * scene is never dead. A single eased `follow` scalar crosses between the two,
 * target AND rate, so the pointer arriving or leaving never steps the ring.
 *
 ...
 * from a ref inside a raw rAF loop. Never calls loseContext() — getContext()
 * hands back the SAME force-lost context on the next StrictMode mount and
 * renders black. Density rebuilds the buffers and the textures, never the
 * context.
 *
 ...
 *
 * It was 0.1 — the same term the free wander uses. Measured against the
 * geometry that is 83% of the cursor's ENTIRE vertical travel (±0.121 at the
 * shipped camera) and 72% of the visible half-height, so the ring spent most of
 * its time somewhere other than under the pointer and the noise, not the
 * cursor, was driving it. At 0.01 it is 8% of the cursor's travel: a live edge
 * on the ring, not a second, louder input.
 */
...
export default function CursorRingField(props: CursorRingFieldProps) {
    ...
    // Seed the ref on the first render, then keep it fresh from an effect —
    // never write a ref during render (react-hooks/refs). The no-dep effect
    // runs after every render, so a colour/ring/speed tweak reaches the loop.
    const live = useRef<any>(liveValue)
    useEffect(() => {
        live.current = liveValue
    })

    // Density is the one input that cannot be a plain ref read: it changes how
    // many points exist. It goes through its own ref and a dirty flag, set from
    // an effect (not during render), and the loop rebuilds buffers + textures.
    ...
    useEffect(() => {
        ...
        // A canvas whose context is lost paints an OPAQUE sheet. This one is
        // full-bleed behind the hero, so a driver reset would white out the
        // page. preventDefault keeps the context restorable; hiding the canvas
        // means a lost one can never cover the content either way.
        ...
        // (No canvas `pointerleave` here on purpose: in the hero, content sits
        // layered above the canvas, and leave-fires from that overlay would
        // keep dropping the ring back to its wander while the cursor is still
        // inside the frame. The bounds test in onMove is the only authority.)
